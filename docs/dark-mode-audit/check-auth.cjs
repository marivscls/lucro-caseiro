...
const out = path.resolve(__dirname, "auth");
  ...
  try {
    for (const width of [390]) {
      ...
      await context.addInitScript(
        ({ session, key, id }) => {
          localStorage.removeItem(key); localStorage.setItem("themeMode", "dark");
      ...
      const routes = ["/login", "/register", "/reset-password#access_token="+token+"&refresh_token=local-test-only&type=recovery"];
      const report = [];
      for (const screen of routes) {
        ...
        try {
          ...
          const file = new URL(screen,base).pathname.replaceAll("/", "-").slice(1) + "-dark.png";
          await page.screenshot({ path: path.join(out, file) });
          const result = {screen: new URL(screen,base).pathname, resolvedPath: new URL(page.url()).pathname, file, errors:[...errors], overflow: await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), text};
          report.push(result);
          console.log(new URL(screen,base).pathname, errors.length ? errors.join("; ").slice(0,200) : "rendered", text.slice(0,70).replaceAll("\n"," "));
