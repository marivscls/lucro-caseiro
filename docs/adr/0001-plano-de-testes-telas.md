...
10. ✅ **6.10 — Acessibilidade (toque).** Criado o componente `Chip` em `@lucro-caseiro/ui` (toque mínimo 44dp, `hitSlop`, `accessibilityRole`/`accessibilityState`). Migrados os chips de filtro de Vendas, de forma de pagamento (edição) e os status da Agenda para o `Chip`. Ações de texto ("Recebi", "Remover logo") já têm `hitSlop`; `Button` já garante 48/56dp. _Pendente (não-bloqueante): auditoria formal de contraste do tema (WCAG AA) — o tema do design system já foi desenhado para AA._
