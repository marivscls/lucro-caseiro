...
.store-card h1{margin:0;color:var(--storefront-primary);font-family:var(--font);font-size:clamp(1.35rem,5.4vw,1.7rem);font-weight:800;line-height:1.2;letter-spacing:-.03em;overflow-wrap:anywhere;word-break:break-word}
.introduction{margin:8px auto 0;max-width:36ch;color:var(--storefront-muted);font-size:.9rem;font-weight:500;line-height:1.4;white-space:pre-line;overflow-wrap:anywhere}
...
.highlight-card{flex:none;width:min(168px,calc(50vw + 12px));scroll-snap-align:start;overflow:hidden;border:1px solid color-mix(in srgb,var(--storefront-action) 12%,white);border-radius:var(--radius-item);background:var(--white);box-shadow:var(--shadow-soft)}
...
.highlight-card h3{margin:0;color:var(--storefront-primary);font-size:.82rem;font-weight:800;line-height:1.3;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden;overflow-wrap:anywhere;word-break:break-word;min-height:2.6em}
...
.storefront-card .card-hit{display:flex;flex-direction:column;align-items:stretch;height:100%;width:100%;padding:0;text-decoration:none;color:inherit;border:0;background:transparent;text-align:left;cursor:pointer}
.storefront-card .item-image,.cards-compact .storefront-card .item-image{width:100%;aspect-ratio:1;height:auto;overflow:hidden;background:var(--blush);border-radius:0;flex:none}
.storefront-card .item-image.is-placeholder,.cards-compact .storefront-card .item-image.is-placeholder{aspect-ratio:5/4}
.item-image img{width:100%;height:100%;object-fit:cover;display:block}
.item-image.is-failed img{display:none}
.item-placeholder{display:grid;place-items:center;min-height:0;height:100%;color:var(--storefront-action);background:radial-gradient(circle at 50% 42%,color-mix(in srgb,var(--storefront-action) 12%,white) 0%,var(--blush) 70%)}
.item-placeholder-mark{width:48px;height:48px;border-radius:50%;display:grid;place-items:center;background:#fff;box-shadow:0 4px 12px rgba(36,24,30,.08);color:var(--storefront-action)}
.item-placeholder-mark svg{width:22px;height:22px}
.storefront-card .item-body,.cards-compact .storefront-card .item-body{min-width:0;display:flex;flex-direction:column;justify-content:flex-start;padding:10px 12px 12px;gap:4px}
.item-category{margin:0;color:var(--storefront-action);font-size:.68rem;font-weight:800;letter-spacing:.04em;text-transform:uppercase}
.highlight-card .item-category{display:none}
.storefront-card .item-body h3{margin:0;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden;overflow-wrap:anywhere;word-break:break-word;color:var(--storefront-primary);font-family:var(--font);font-size:.88rem;font-weight:800;line-height:1.3;min-height:2.6em}
.item-description{display:none}
.item-description.expanded{display:block;margin:6px 0 0;color:var(--warm);font-size:.8rem}
.item-price{display:flex;flex-direction:column;align-items:flex-start;gap:1px;margin:2px 0 0;color:var(--storefront-primary);font-size:.9rem;font-weight:800}
.price-from{font-size:.68rem;font-weight:700;color:var(--warm);line-height:1.2}
...
dialog{width:min(520px,calc(100% - 28px));max-height:calc(100dvh - 28px);overflow:visible;border:0;border-radius:16px;padding:0;background:#fff;color:var(--ink);box-shadow:0 24px 60px rgba(36,24,30,.22)}
...
dialog form,.dialog-panel{display:grid;gap:14px;padding:22px 24px;max-height:calc(100dvh - 28px);overflow-x:visible;overflow-y:auto}
...
dialog input:not([type=hidden]),dialog select,dialog textarea,.picker-field{width:100%;max-width:100%;min-width:0;min-height:48px;border:1px solid var(--storefront-border);border-radius:10px;padding:12px;background:var(--cream);color:var(--ink);box-sizing:border-box}
dialog textarea{min-height:84px;resize:vertical}
.form-row{display:grid;grid-template-columns:1fr;gap:14px}
.form-row>*{min-width:0}
.picker-field{display:flex;align-items:center;justify-content:space-between;gap:10px;font:inherit;font-weight:700;text-align:left;cursor:pointer}
.picker-field.is-empty{color:var(--warm);font-weight:600}
.picker-field svg{width:22px;height:22px;color:var(--storefront-primary)}
#booking-calendar{width:min(520px,calc(100% - 28px));border:0;border-radius:20px;padding:18px;background:#fff;color:var(--ink);box-shadow:0 24px 60px rgba(36,24,30,.22)}
.lc-cal-head{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:12px}
.lc-cal-head>button{min-width:44px;min-height:44px;border:0;border-radius:12px;background:transparent;color:var(--storefront-primary);display:grid;place-items:center;cursor:pointer}
#cal-prev svg{transform:scaleX(-1)}
#cal-title{flex:1;display:flex;align-items:center;justify-content:center;gap:6px;color:var(--ink);font-weight:800;font-size:1.05rem}
#cal-title.is-years svg{transform:rotate(180deg)}
.lc-cal-week,.lc-cal-grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr))}
.lc-cal-week{margin-bottom:4px;color:var(--storefront-primary);font-size:.72rem;font-weight:800;text-align:center}
.lc-cal-day{height:42px;border:0;background:transparent;color:var(--ink);font:inherit;font-weight:700;display:grid;place-items:center;cursor:pointer}
.lc-cal-day span{width:38px;height:38px;border-radius:50%;display:grid;place-items:center}
.lc-cal-day.is-muted{opacity:.45}
.lc-cal-day.is-selected span{background:var(--storefront-primary);color:#fff}
.lc-cal-day.is-today:not(.is-selected) span{border:1.5px solid var(--storefront-primary)}
.lc-cal-years{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px}
.lc-cal-year{min-height:48px;border:0;border-radius:12px;background:var(--cream);color:var(--ink);font:inherit;font-weight:800;cursor:pointer}
...
@media(max-width:389px){
  ...
  .highlight-card{width:min(168px,72vw)}
}
