...
- 2026-07-11: **WP1 monetização — PRD melhorias pré-lançamento (item 1.1, 2.1–2.4)**:
  - **Cancelamento funcional** (`plans.tsx`): o botão "Cancelar assinatura" (antes um alerta "em breve") abre a gestão de assinatura da loja via `Linking.openURL` — Android `https://play.google.com/store/account/subscriptions?package=<androidPackage>`, iOS `https://apps.apple.com/account/subscriptions`. Fallback: se o `Linking` falhar, `showAlert` orienta a contatar o suporte. `UserProfile` não guarda o `productId` da loja, então o link Android não inclui `?sku=`.
  - **`exportBasic` no Essencial** (ADR-0005): nova `PlanFeature` na matriz de `@lucro-caseiro/contracts` (ver `ai.context.api.md` de subscription/finance). `TIER_BENEFITS.essential` (`plan-benefits.ts`) ganhou "Exporta o resumo do mês em PDF" e "Fornecedores: até 3 (ilimitado no Profissional)" — usado tanto no `Paywall` quanto nos cards de `plans.tsx` (que agora referenciam `TIER_BENEFITS.essential`/`.professional` em vez de listas duplicadas).
