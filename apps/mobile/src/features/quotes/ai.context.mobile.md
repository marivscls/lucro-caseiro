...
- 2026-09-16: o Continuar da etapa de itens exige descrição, quantidade maior que zero e preço preenchidos em cada item. Quantidade vazia gera `NaN` em `parseNumber`, então a comparação usa `!(parseNumber(qtd) > 0)` em vez de `<= 0`.
