    ...
    for (const width of (process.env.FORM_WIDTHS || "320,390,500")
      ...
      await context.route("**/*", async (route) => {
        ...
        if (p === "/insights")
          result = {
            ...
            totalSales: 20,
            monthlyRevenue: [
              { month: "2026-04", revenue: 8200.5, salesCount: 4 },
              { month: "2026-05", revenue: 12340, salesCount: 6 },
              { month: "2026-06", revenue: 9800, salesCount: 5 },
              { month: "2026-07", revenue: 15420.75, salesCount: 8 },
              { month: "2026-08", revenue: 18700, salesCount: 9 },
              { month: "2026-09", revenue: 21500.3, salesCount: 12 },
            ],
            topProducts: [
              { productId: "product-0", name: "Bolo de chocolate", quantity: 32, revenue: 1280 },
              { productId: "product-1", name: "Brigadeiro artesanal", quantity: 120, revenue: 960 },
            ],
            topClients: [
              { clientId: "client-0", name: "Ana Souza", totalSpent: 2450, salesCount: 6 },
              { clientId: "client-1", name: "Carla Lima", totalSpent: 1830, salesCount: 4 },
            ],
          };
      ...
      for (const route of routes) {
        ...
        try {
          ...
          if (route === "tabs/new-sale") {
            const shot = async (state) => {
              await page.screenshot({
                path: path.join(out, `new-sale-390-${state}.png`),
                animations: "disabled",
              });
              const edges = await page.evaluate(() => {
                ...
                for (const el of document.querySelectorAll("div,span,button")) {
                  ...
                  const s = getComputedStyle(el);
                  if (s.visibility === "hidden") continue;
                  if (r.right > maxRight && r.right <= innerWidth + 2) {
                    maxRight = r.right;
                    widest = (el.textContent || "").trim().slice(0, 40);
                  }
                }
                return { innerWidth, maxRight: Math.round(maxRight), widest };
              });
              console.log(JSON.stringify({ route, state, edges }));
            };
            await page.getByText("Venda avulsa", { exact: true }).first().click();
            await page.waitForTimeout(600);
            await shot("step2");
            const geometry = await page.evaluate(() => {
              const cards = Array.from(
                document.querySelectorAll('[aria-label^="Produto "]'),
              ).map((el) => {
                const r = el.getBoundingClientRect();
                return {
              ...
              return { innerWidth, cards: cards.slice(0, 4) };
            });
            console.log(JSON.stringify({ route, geometry }));
            await page.getByText("Bolo de chocolate", { exact: true }).first().click();
            await page.waitForTimeout(600);
            await shot("step2-selected");
            await page.getByRole("button", { name: "Pagamento", exact: true }).last().click();
            await page.waitForTimeout(600);
            await shot("step3");
            await page.getByRole("radio", { name: "Pix", exact: true }).last().click();
            await page.waitForTimeout(400);
            await page.getByRole("button", { name: "Revisar venda", exact: true }).last().click();
            await page.waitForTimeout(600);
            await shot("step4");
          }
