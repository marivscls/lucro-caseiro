...
- 2026-07-11: **WP1 monetização — PRD melhorias pré-lançamento (item 1.1, 2.1–2.4)**:
  - **Cancelamento funcional** (`shared/utils/subscription-management.ts`): o botão "Cancelar assinatura" abre a gestão no Google Play no Android; iOS/Web, que usam Stripe, abrem um pedido de cancelamento ao suporte. O mesmo fluxo é reutilizado antes da exclusão de uma conta paga. Fallback: se o `Linking` falhar, `showAlert` orienta a contatar o suporte. `UserProfile` não guarda o `productId` da loja, então o link Android não inclui `?sku=`.
