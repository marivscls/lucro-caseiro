...
- 2026-06-16: **vendas ilimitadas no free** — `FREE_PLAN_LIMITS.maxSalesPerMonth = Infinity` (era 30) e o `freemiumGuard("sales")` foi **removido** do `POST /api/v1/sales`. Registrar venda é a ação central diária; o free fica "básico" pelos outros limites (clientes/produtos 20, receitas 5, embalagens 3) + features Premium. O `/limits` retorna `maxSalesPerMonth: Infinity` no free; o app oculta vendas do "uso atual" (só mostra recursos com limite finito).
